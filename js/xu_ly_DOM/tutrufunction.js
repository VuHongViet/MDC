...
{ct[i]=new Array();
	for (var j=0;j<4;j++)
ct[i][j]=cantang[achi[i]][j]};

//ct la can tang cua cac tru
...
{dt[i]=new Array();
	for (var j=0;j<4;j++)
dt[i][j]=cantang[achi[i]][j]};
//dt sẽ là điểm của các can tang
...
//Gán ct cho các  can tàng trong chi, dt cho cường nhược. của nó
function annhap(){
//ansaotutru(ct[1],3,1,2);
//ansaotutru(ct[2],3,2,2);
//ansaotutru(ct[3],3,3,2);
//ansaotutru(ct[4],3,4,2);

//ansaotutru(dt[1],3,1,2);
//ansaotutru(dt[2],3,2,2);
//ansaotutru(dt[3],3,3,2);
//ansaotutru(dt[4],3,4,2);

//ansaotutru(diemthiencan[1],3,1,2);
//ansaotutru(diemthiencan[2],3,2,2);
//ansaotutru(diemthiencan[3],3,3,2);
//ansaotutru(diemthiencan[4],3,4,2);
}
//annhap();
...
{	
		for (var i=1; i<5;i++)
		{
					//context.fillText(can[ct[i][1]], 30 + w / 2+i*w, 600 + hh + 75);
