...
gio = check(parseInt(doc.getElementById('ID_GIO').value));
    ...
    if (!doc.getElementById('ID_THANGXEMNHUAN').checked)
        ...
        nhuanxem = 1;
   if (!doc.getElementById('laso1').checked)//ngang
        laso = 0;
...
function sync() {//nếu dương nạp sang âm và ngược lại
...
function setup() {	
    ...
    // 2 tính toán tất cả các dữ liệu lịch của năm sinh và năm xem)
    {
...
{if (laso!=1)
		...
		var thunghiem;
		for (var i=0; i<1; i++)
		{thunghiem= bacot(cannam,chinam,thang,ngay,gio,cannamxem,chinamxem,canthangxem,chithangxem,canngayxem,chingayxem,cangioxem,chigioxem,ngayxem, thangxem, namxem);	
			document.getElementById('canvasImg').src = thunghiem;
			var tutrudataimage= tutru(acan,achi);
		...
		//document.getElementById('canvasImg4').src = tutru(acan,achi);

		}
else
		{
	//	document.getElementById('canvasImg').src = bacotdoc();
