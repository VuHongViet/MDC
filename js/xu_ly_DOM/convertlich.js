...
function NewMoon(k) {
    ...
    F = 21.2964 + 390.70050646 * k - 0.0016528 * T2 - 0.00000239 * T3; // Moon's argument of latitude
...
function OnCheckData() {
    ...
    if (document.getElementById('ID_TEN').value == '') {
        alert('Chưa nhập tên');
        return false;
    }
    if (document.getElementById('ID_LICH').selected) {
        if (ngay > DaysOfMonth(thang, nam)) {
            alert('Chọn ngày không đúng');
            return false;
        }
    }
